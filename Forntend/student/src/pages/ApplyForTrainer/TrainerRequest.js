...
const [submissionMessage, setSubmissionMessage] = useState('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmissionMessage('Your request for a trainer has been successfully sent!');

      } else {
        console.error('Failed to submit trainer request.');
        setSubmissionMessage('Failed to submit trainer request. Please try again.');

      }
    } catch (error) {
      console.error('Error submitting trainer request:', error);
      setSubmissionMessage('Error submitting trainer request. Please try again later.');
  ...
  return (
    <div className="ApplyTrainer">
      ...
      {submissionMessage && (
        <p style={{ textAlign: 'center', color: submissionMessage.includes('successfully') ? 'green' : 'red' }}>
          {submissionMessage}
        </p>
      )}
