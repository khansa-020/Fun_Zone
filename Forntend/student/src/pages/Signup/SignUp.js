...
const SignUp = () => {
  ...
  const handleChange = (e) => {
    // Clear password error when the password field changes
  ...
  const isPasswordValid = () => {
    // Password must have at least 8 characters with at least one alphabet, one number, and one special character
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isPasswordValid()) {
      setPasswordError('Password must have at least 8 characters with at least one alphabet and one number.');
      return;
    }
